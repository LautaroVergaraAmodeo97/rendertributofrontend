...
const CATEGORIA_MAP = {
  A: { label: "Categoría A", descripcion: "Riesgo muy bajo", color: "#00e096", bg: "#00e09615" },
  B: { label: "Categoría B", descripcion: "Riesgo bajo", color: "#64d8a0", bg: "#64d8a015" },
  C: { label: "Categoría C", descripcion: "Riesgo medio / Nuevas altas", color: "#f0b429", bg: "#f0b42915" },
  D: { label: "Categoría D", descripcion: "Riesgo alto", color: "#ff4d6d", bg: "#ff4d6d15" },
  E: { label: "Categoría E", descripcion: "Riesgo muy alto", color: "#c0392b", bg: "#c0392b15" },
};

function calcularCategoria(periodos) {
  if (!periodos || periodos.length === 0) return null;

  // Tomar solo el período más reciente
  const periodoReciente = [...periodos].sort((a, b) => b.periodo - a.periodo)[0];
  const entidades = periodoReciente?.entidades || [];

  if (entidades.length === 0) return null;

  const situaciones = entidades.map((e) => e.situacion);
  const maxSituacion = Math.max(...situaciones);
  const cantidadEntidades = entidades.length;

  if (maxSituacion === 5) return "E";
  if (maxSituacion === 4) return "D";
  if (maxSituacion === 3) return cantidadEntidades >= 3 ? "D" : "C";
  if (maxSituacion === 2) return cantidadEntidades >= 3 ? "C" : "B";
  return "A";
}
...
function LineChart({ periodos }) {
  ...
  useEffect(() => {
    ...
    // Ordenar de más antiguo (izquierda) a más reciente (derecha)
  ...
  useEffect(() => {
    ...
    chartRef.current = new Chart(ref.current, {
      ...
      options: {
        ...
        scales: {
          x: {
            ticks: { display: false }, // Sin etiquetas en el eje X
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto" }}>
        ...
        {resultado && (
          <div>
            {/* Info titular */}
            {(() => {
              const categoria = calcularCategoria(resultado.periodos);
              const cat = categoria ? CATEGORIA_MAP[categoria] : null;
              return (
                <div style={{ background: "#111827", border: "1px solid #1e2d45", borderRadius: 16, padding: 24, marginBottom: 24, display: "flex", gap: 32, alignItems: "center", flexWrap: "wrap" }}>
                  <div>
                  ...
                  {cat && (
                    <>
                      <div style={{ width: 1, height: 40, background: "#1e2d45" }} />
                      <div>
                        <p className={dmMono.className} style={{ color: "#64748b", fontSize: 11, letterSpacing: 2, marginBottom: 8 }}>PERFIL DE RIESGO</p>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 10, background: cat.bg, border: `1px solid ${cat.color}33`, borderRadius: 10, padding: "8px 16px" }}>
                          <span style={{ fontSize: 22, fontWeight: 800, color: cat.color, fontFamily: "'Syne', sans-serif" }}>
                            {categoria}
                          </span>
                          <div>
                            <p style={{ color: cat.color, fontSize: 13, fontWeight: 700, margin: 0 }}>{cat.label}</p>
                            <p className={dmMono.className} style={{ color: "#64748b", fontSize: 10, margin: 0 }}>{cat.descripcion}</p>
                          </div>
                        </div>
                      </div>
                    </>
